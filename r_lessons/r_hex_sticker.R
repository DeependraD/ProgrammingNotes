...
purrr::map_int(list(x, y), length)

hex <- data.frame(x, y)
hex2 <- data.frame(x = 1.4 * x, y = 1.4 * y)

line_points <- data.frame(
  lpx = c(-0.5, -0.3, -0.05, 0.05, 0.3, 0.5),
  lpy = c(0, 0.4, 0.9, 0.30, 0.7, 1)
)

bg_col <- c("#F2F2F2")
l_col <- c("#424242")
p_col <- c("#A8D6FF", "#51A7F9", "#0365C0", 
           "#164F86", "#002452", "#000643")

require(ggplot2)
ggplot(hex, aes(x, y)) +
  geom_line(aes(x = lpx, y = lpy), data = line_points, 
            size = 4, color = l_col) +
  geom_point(aes(x = lpx, y = lpy), data = line_points, 
             size = 23, color = p_col) +
  ...
  geom_path(aes(x, y), data = hex2, size = 75, color = "white", 
            lineend = "square") +
